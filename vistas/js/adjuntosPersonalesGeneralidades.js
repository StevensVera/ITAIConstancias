...
var table = $(".tablasAdjuntosDatosPersonales").DataTable({
   
  "ajax":"ajax/datatable-adjuntosPersonalesGeneralidades.ajax.php?perfilOculto="+perfilOculto,
  "deferRender": true,
  "retrieve": true,
  "processing": true,
   "scrollY": 450,
...
$('.tablasAdjuntosDatosPersonales tbody').on( 'click', '.btnImprimirConstancia', function () {  
  
  var idAdjuntos =  $(this).attr("idAdjuntos");

  window.open("extensiones/tcpdf/pdf/ConstanciaPersonalesGeneralidades.php?id="+idAdjuntos, "_blank");
...
$(".tablasAdjuntosDatosPersonales").on("click", ".btnEditarAdjuntos", function(){
  ...
  $.ajax({
    url: "ajax/adjuntosPersonales.ajax.php",
    method: "POST",
        data: datos,
        cache: false,
      contentType: false,
...
/*==================================================================
 ...
 $(".tablasAdjuntosDatosPersonales").on("click", ".btnEliminarAdjuntos", function(){
    ...
    type: 'warning',
        ...
        if (result.value) {
          ...
          window.location = "index.php?ruta=taller-datos-generalidades&idAdjuntos="+idAdjuntos+"&nombre="+nombre+"&correo="+correo+"&taller="+taller;
